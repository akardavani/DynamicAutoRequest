...
{
    public  class SahraRequest : BaseRequest
    {
        public  async Task<(string text, LogJson jsonLog)> Send(TimeSpan delay, OrderData orderData)
        {
            try
            {
                ...
                var request = new HttpRequestMessage(HttpMethod.Post, $"{orderData.OriginUrl}/api/v2/orders/NewOrder".Replace("https://", "https://api-"));
                ...
                request.Headers.Add("x-sessionId", $"{orderData.SessionId}");
