...
{
    public class AsaTraderRequest : IOmsRequest
    {
        private readonly EasyTraderOrderRequestSnapshot snapshot;

        public AsaTraderRequest()
        {
            string jsonFolderPath = Path.Combine(Environment.CurrentDirectory, "Json");
            snapshot = JsonConvertor.ReadJsonData<EasyTraderOrderRequestSnapshot>(JsonFileNames.AsaTraderOrderRequestSnapshot, jsonFolderPath);
        }
